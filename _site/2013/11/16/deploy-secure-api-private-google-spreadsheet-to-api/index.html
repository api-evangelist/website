...
image: https://s3.amazonaws.com/kinlane-productions2/bw-icons/bw-google-docs.png
...
<p>Slim is pretty straightforward to work with, to add each API endpoint you just add a single PHP file under methods. For this how-to guide we are going to add a simple endpoint from our private spreadsheet products data store.
...
<p>There are any number of reasons you would want to secure an API driven from a private Google Spreadsheet file, to offer advanced search, filtering or just to track on who is accessing resources. This solution just uses 3Scale to secure the API interface, requiring an AppID and Appkey to use the API.&nbsp;
<p>Hopefully that demonstrates a simple approach to securing an API that is driven from a private Google Spreadsheet file.
