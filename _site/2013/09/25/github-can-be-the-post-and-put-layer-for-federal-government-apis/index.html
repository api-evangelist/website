...
<p><img src="https://s3.amazonaws.com/kinlane-productions2/api-evangelist/augmented-post.png" alt="" width="250" align="right" />
<p>I'm playing with different approaches to rapidly design, develop, deploy and manage APIs using Github. While about 90% of what I'm building runs on Github, there is still about 10% that runs on Amazon EC2.
...
<p><img src="https://s3.amazonaws.com/kinlane-productions2/bw-icons/bw-government.jpg" alt="" width="250" align="right" />
<p>If you look at my recent <a href="https://kinlane.github.io/dev-hub/index.html">Dev Hub</a> prototype, you can browse the API interface, thanks to <a href="https://developers.helloreverb.com/swagger/">Swagger</a>, and when you make API calls to the endpoints via Amazon EC2, the REST interface is just acting as a search, filter and REST facade for the JSON files that are actually stored on Github--eliminating the need for a database backend.
<p>This approach allows me to develop light-weight REST facades for JSON data stores as well as for other APIs. I'm playing around with different ways that I can use this thought process to push government APIs to the next level, and building on my earlier thoughts today on the <a href="http://apievangelist.com/2013/09/25/a-huge-need-for-writeable-apis-in-government/">need for writable APIs in federal government</a>.
<p>I'm evolving earlier designs I have in my archives of <a href="http://apievangelist.com/2013/03/17/beyondget-or-otherverbs-an-augmented-api-platform-/">BeyondGET or OtherVerbs, an Augmented API Platform</a>, where I'm looking to provide an augmented layer that makes existing APIs writable.  Marrying these legacy thoughts with my current approaches using Github plus EC2 APIs, I strongly feel that Github has huge potential for being a POST, PUT and DELETE layer for federal government APIs.
<p>Using a REST facade on EC2 I can easily proxy existing government APIs, then using a swagger definition I could seamlessly weave in a POST, PUT and DELETE layer that would write to JSON files hosted at Github using the Github API.
