...
image: https://s3.amazonaws.com/kinlane-productions2/api-evangelist/reliefweb/reliefweb-logo.jpg
---

<p><a href="http://reliefweb.int" target="_blank"><img style="padding: 15px;" src="https://s3.amazonaws.com/kinlane-productions2/api-evangelist/reliefweb/reliefweb-logo.jpg" alt="" width="200" align="right" /></a>
<p>I was introduced to the <a href="http://reliefweb.int/help/api">ReliefWeb API</a> last week, which provides information including reports, jobs, training, countries, and source details on global crises and disasters going back to 1996.
<p>The ReliefWeb API follows a trend of organizations realizing not just the potential of APIs for delivering content to mobile applications, but also the benefits of opening up publicly. As the site states:
...
<p>Since then they have normalized the interfaces for public consumption, hoping that &ldquo;developers can build tools that increase exposure for valuable content and facilitate analysis for better decisions.&rdquo;
<p>The ReliefWeb API provides seven valuable API endpoints:
<ul class="mainlist">
...
<p>The design of the API is pretty well thought out, providing some common, as well as some forward thinking technical elements:
<ul class="mainlist">
...
<p><strong>Looking Forward</strong><br /> It is always hard to critique new APIs from governmental or non-profit organizations, because I'm just happy to see any API come out the door. This means the iteration cycles start, and things will only get better (potentially).
<p>I could recommend <a href="http://management.apievangelist.com/building-blocks.html">other building blocks</a> like code samples, idea and application showcase and social elements like usage of Github and Twitter, but I know they are just getting going and these things will come.
<p>Overall I&rsquo;m excited about the potential around the ReliefWeb API, that the organization has seen the benefits of APIs in their own mobile strategy, but also understands the potential of opening up these resources to the public.
<p>Seeing deployments like the ReliefWeb API remind me that we can truly tackle many of the big problems we face in the world, rather than many of the stupid, first world problem API solutions and applications we see out of Silicon Valley.
