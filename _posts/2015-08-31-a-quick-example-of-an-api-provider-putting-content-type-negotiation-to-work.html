...
<p><img style="padding: 15p;" src="https://s3.amazonaws.com/kinlane-productions2/bw-icons/bw-handshake.png" alt="" width="250" align="right" />
...
<p>When integrating with the GCIS API, it all begins with some&nbsp;basic JSON, but then look at all the other linked data goodness in there! JSON-LD, RDF, Turtle!! Discovering APIs that support robust content type negotiation like this can be hard to find, most APIs don't talk about it, so even if they do, there is no way to know about it. Thankfully the&nbsp;GCIS API shares it via email, as well as describes in the <a href="/admin/blog/view-source:http:/data.globalchange.gov/api_reference.json">GCIS Swagger definition</a>, which is how I learned about it.&nbsp;
<p>Content-Type negotiation is one of those HTTP literacy areas that need a lot of discussion, helping on-board API newbies to the power and possibilities that are present here, and that you can go beyond just plain ol JSON, and receive much richer API responses.
