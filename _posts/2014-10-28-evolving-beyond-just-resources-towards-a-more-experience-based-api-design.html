...
image: https://s3.amazonaws.com/kinlane-productions2/bw-icons/bw-experience-design.png
---

<p><img src="https://s3.amazonaws.com/kinlane-productions2/bw-icons/bw-experience-design.png" alt="" width="325" align="right" />
<p>I really love the thought process around API design, and I am always looking for ways to push forward my craft. Telling stories is one way I do this, forcing me to make what I&rsquo;m doing understandable to a wider audience, which is also the exact thing I&rsquo;m trying to do with my API designs&mdash;mind blown!
<p>An example of this in action is with my link API, which I use in multiple places across my API monitoring system. The Link API has a pretty basic design, that reflect the resource, and its underlying database structure:
<ul class="mainlist">
<li>/link/ (GET, POST, PUT, DELETE)</li>
<li>Title, Description, and URL</li>
</ul>
<p>I use this API in numerous ways, the original API design is very much based upon the resource that it is, a link. As a developer who understands REST, and very much comes from a world of databases, this works just fine. However much of the magic around my usage of links is defined in my code, and not in my API interface, something I&rsquo;d like to change.
<p>After having two separate conversations with folks about my link API, I experienced two very different motivations on using said link system, and came with very different perspectives on how to use mu ink API, that had nothing to do with the database or resource this API was born from.
